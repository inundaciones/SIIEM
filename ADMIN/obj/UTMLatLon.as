package obj
{
	public class UTMLatLon
	{
		...
		public function LatLonToUTM():void{
			...
			var NU:Number = A/Math.sqrt(1-Math.pow((E*Math.sin(latR)),2));
			
			var S:Number=A0*latR - B0*Math.sin(2*latR) + C0*Math.sin(4*latR) - D0*Math.sin(6*latR) + E0*Math.sin(8*latR);
			
			var K1:Number = S*k0;
			var K2:Number= k0*NU*Math.sin(latR)*Math.cos(latR)/2;
			var K3:Number = ((NU*Math.sin(latR)*Math.pow(Math.cos(latR),3))/24)*(5-Math.pow(Math.tan(latR),2)+9*E2*Math.pow(Math.cos(latR),2)+4*Math.pow(E2,2)*Math.pow(Math.cos(latR),4))*k0;
			
			var K4:Number = k0*NU*Math.cos(latR);
			var K5:Number =Math.pow(Math.cos(latR),3)*NU/6*(1-Math.pow(Math.tan(latR),2)+E2*Math.pow(Math.cos(latR),2))*k0;
			// (k0*NU*Math.pow(Math.cos(latR),3)/6)*(1 - Math.pow(Math.tan(latR),2) + E0*E0*Math.pow(Math.cos(latR),2));
			
			
			this.UTMy=K1+K2*Math.pow(P,2)+K3*Math.pow(P,4);
			this.UTMx=500000+(K4*P+K5*Math.pow(P,3));
