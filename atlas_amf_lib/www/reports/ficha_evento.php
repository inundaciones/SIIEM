<?php		
	...
	$y=613;
	$restText=$pdf->addTextWrap(36,$y,263,8,"<b>Colonias y/o barrios: </b>".utf8_decode($row->COLONIA));
	while(str_word_count($restText)>0){
		$y-=12;
		$restText=$pdf->addTextWrap(36,$y,42,8,utf8_decode($restText));
	}
	$pdf->addText(36,577,8,"<b>Delimitación del área (calles):  </b>");
	$pdf->addText(36,565,8,"<b>Al norte:</b>");
	$pdf->addText(94,565,8,utf8_decode($row->L_N));
	$pdf->addText(36,553,8,"<b>Al sur:</b>");
	$pdf->addText(94,553,8,utf8_decode($row->L_S));
	$pdf->addText(36,541,8,"<b>Al oriente:</b>");
	$pdf->addText(94,541,8,utf8_decode($row->L_O));
	$pdf->addText(36,529,8,"<b>Al poniente: </b>");
	$pdf->addText(94,529,8,utf8_decode($row->L_P)); 
	...
	$pdf->ezText("<b>Causas: </b>".utf8_decode($causas),8);
	$pdf->ezText("\n<b>Descripcion:</b> ".utf8_decode($row->DESCRIPCION) ,8);
	...
	while($row2=mysql_fetch_object($rs2)){
		$y-=12;
		if($row2->TIPO2='O'){
			$restText1=$pdf->addTextWrap(36,$y,330,8,"- ".utf8_decode(toTextObra($row2->ID_ACCION)));
		}else{
			$restText1=$pdf->addTextWrap(36,$y,330,8,"- ".utf8_decode(toTextAccion($row2->ID_ACCION)));
		}
		$restText3=$pdf->addTextWrap(378,$y,118,6,utf8_decode(toTextDependencia($row2->ID_DEPENDENCIA)));
		$wC=$pdf->getTextWidth(8,"$".number_format ($row2->COSTO,2)); 
		$pdf->addText(570-$wC,$y,8,"$".number_format ($row2->COSTO,2));
	...
	while($row2=mysql_fetch_object($rs2)){
		$y-=12; 
		$restText3=$pdf->addTextWrap(36,$y,535,8,"<b>Usuario </b> <i>".toTextLogin($row2->URI_USER)."</i>:   ".utf8_decode($row2->OBSERVACION));
	...
	try{
		...
		$y=$yi=690; 
		plecar($pdf,$row);
		$pdf->addJpegFromFile('../files/snapshots/'.$_POST['ID'].'.jpg',10,400,600,274);
